...
#include "BasicClientSortedDelayTest.h"
...
Define_Module(BasicClientSortedDelayTest);

BasicClientSortedDelayTest::BasicClientSortedDelayTest() {
...
int BasicClientSortedDelayTest::numInitStages() const {
    return 2;
}

void BasicClientSortedDelayTest::initialize(int stage) {
    if (stage == 0) {
        HostBase::initialize();
...
bool BasicClientSortedDelayTest::r_transmit(cMessage* msg,
...
void BasicClientSortedDelayTest::dispatchHandler(cMessage *msg) {
...
void BasicClientSortedDelayTest::loadContent(cMessage* msg) {
    if (!triggered) {
        triggered = true;
        start = simTime();
...
void BasicClientSortedDelayTest::onInventory(CANMessage* msg) {
...
void BasicClientSortedDelayTest::onDataReply(ChordMessage* msg) {
    if (!crc.empty()) {
        hops += msg->getHop();
        string content = msg->getContent();
...
void BasicClientSortedDelayTest::startPlay(cMessage* msg) {
...
void BasicClientSortedDelayTest::move(cMessage* msg) {
...
void BasicClientSortedDelayTest::changePosition() {
...
void BasicClientSortedDelayTest::displayPosition() {
...
void BasicClientSortedDelayTest::finish() {
    if (circle != NULL) {
        getParentModule()->getParentModule()->getCanvas()->removeFigure(circle);
        delete circle;
    }
