...
bool Client::r_transmit(cMessage* msg, unsigned long destID) {
...
void Client::refreshContent(cMessage* msg) {
    clearCache();
    loadContent();
...
void Client::onDataRetrievalTimeout(cMessage* msg) {
...
void Client::onInventory(CANMessage* msg) {
...
void Client::onContent(CANMessage* msg) {
    string content = msg->getContent();
...
void Client::onDataReply(ChordMessage* msg) {
    if (!crc.empty()) {
        hops += msg->getHop();
...
void Client::startPlay(cMessage* msg) {
