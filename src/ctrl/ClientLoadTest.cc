...
Define_Module(ClientLoadTest);

ClientLoadTest::ClientLoadTest() {
...
int ClientLoadTest::numInitStages() const {
    return 2;
}

void ClientLoadTest::initialize(int stage) {
    if (stage == 0) {
        HostBase::initialize();
...
bool ClientLoadTest::r_transmit(cMessage* msg, unsigned long destID) {
...
void ClientLoadTest::dispatchHandler(cMessage *msg) {
...
void ClientLoadTest::refreshContent(cMessage* msg) {
    clearCache();
    loadContent();
...
void ClientLoadTest::clearCache() {
    // Cache management policy: 4 * search radius
    long M = radius * 4;
...
void ClientLoadTest::loadContent() {
    start = simTime();
...
void ClientLoadTest::onInventory(CANMessage* msg) {
...
//    cout << "hops for inventory: " << msg->getHop() << endl;
    ...
    if (content.find(data::DATA_EMPTY) == string::npos) {
        ...
        for (auto elem : inventory["object"]) {
            ...
            if (dist <= radius && history.count(coord) == 0) { // not in the history
                crc[coord] = elem.dump();
            }
        }
    } else {
...
void ClientLoadTest::onContent(CANMessage* msg) {
    string content = msg->getContent();
...
void ClientLoadTest::onDataReply(ChordMessage* msg) {
    if (!crc.empty()) {
        hops += msg->getHop();
...
void ClientLoadTest::startPlay(cMessage* msg) {
...
void ClientLoadTest::move(cMessage* msg) {
...
void ClientLoadTest::changePosition() {
...
void ClientLoadTest::displayPosition() {
